...
class BedrockKnowledgeBase:
    ...
    def __init__(
        ...
        # self.bucket_names = [
        #     f"s3://{d['bucket_name']}/{d['prefix'].rstrip('/')}" if 'prefix' in d and d['prefix'] 
        #     else f"s3://{d['bucket_name']}"
        #     for d in self.data_sources if d['type'] == 'S3'
        # ]

        print("Bucket names:", self.bucket_names)
        ...
        self.encryption_policy_name = f"{self.kb_name}-sp"
        self.network_policy_name = f"{self.kb_name}-np"
        self.access_policy_name = f'{self.kb_name}-ap'
        ...
        self.vector_store_name = f'{self.kb_name}'
        self.index_name = f"{self.kb_name}-index"
        self.graph_id = None
    ...
    def create_data_sources(self, kb_id, data_sources):
        ...
        for idx, ds in enumerate(data_sources):
            ...
            print("S3 inclusionPrefixes", [ds['prefix']] if 'prefix' in ds and ds['prefix'] else ["*.*"])

            s3_data_source_congiguration = {
                    "type": "S3",
                    "s3Configuration":{
                        "bucketArn": "",
                        "inclusionPrefixes": [ds['prefix']] if 'prefix' in ds and ds['prefix'] else ["*.*"],
                        # "inclusionPrefixes":["*.*"] # you can use this if you want to create a KB using data within s3 prefixes.
                        }
                }

            print("S3 Data Source Configuration", s3_data_source_congiguration)
